...
import pickle
import queue
from clase_conductor_2 import conductor
import numpy as np
import time
...
class modo_autonomo(object):
    ...
    def PA(self,nn,frameg,puerto_serial):
        
        puerto_serial = "/dev/ttyACM0"
        conduccion = conductor(puerto_serial,self.cliente)
        print("MODO AUTONOMO")
        ctrl_bucle = True
    
        while ctrl_bucle:
            
            Recepcion_Mensaje = self.cliente.recv(255)
            
            if Recepcion_Mensaje == b'AUTONOMO':
 
                vector_entrada = frameg.get()
                prediccion = nn.predict(vector_entrada)
                conduccion.decision(prediccion)
                                
            elif Recepcion_Mensaje == b'fin del modo autonomo':
                ctrl_bucle = False
                print("Se ha finalizado el modo autonomo")
                break


class modo_cliente_manual(object):
